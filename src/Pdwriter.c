...
size_t PdWriteInteger(FILE *file, PdInteger pValue)
{
    if (!pValue || !file)
        return 0;
    int ct = fprintf(file, "%lld", *pValue);
    return ct > 0 ? ct : 0;
}
...
static size_t PdWriteBytes(FILE *file, struct buffer *pValue, const char *pbegin, const char *pend)
{
    if (!pValue || !file)
        return 0;
    size_t total = 0;
    if (pbegin)
        total = fwrite(pbegin, 1, strlen(pbegin), file);
    total += fwrite(pValue->buffer, 1, pValue->bufferSize - 1, file);
    if (pend)
        total += fwrite(pend, 1, strlen(pend), file);
    return total;
}
size_t PdWriteString(FILE *file, PdString pValue)
{
    return PdWriteBytes(file, pValue, "(", ")");
}
...
{
    return PdWriteBytes(file, pValue, "stream\n", "\nendstream");
}
...
size_t PdWriteIndirectObjRef(FILE *file, PdIndirectObjRef ref)
{
    if (!ref || !file)
        return 0;
    size_t ret = 0;
    ret = fprintf(file, "%lld %lld R", ref->id, ref->generation);
    return ref > 0 ? ret : 0;
}
...
size_t PdWriteStartXref(FILE *file, PdStartXref obj)
{
    if (!obj || !file)
        return 0;
    int tmp = fprintf(file, "startxref\n%lld", *obj);
    if (tmp <= 0)
        return 0;
    return tmp;
}
